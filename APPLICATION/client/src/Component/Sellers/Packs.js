...
function PacksSeller() {
  ...
  const Proack = (id)=>{
    localStorage.setItem('packname','Pack Pro');
    localStorage.setItem('packprice','3000MAD');
    // console.log(name);
    ...
    const ExpertPack = (id)=>{
      localStorage.setItem('packname','Pack Expert');
      localStorage.setItem('packprice','5000MAD');
      // const name=localStorage.getItem('name');
      history.push('/BuyPack');
      }
    ...
    <nav className="w-56 bg-white dark:bg-gray-800 select-none overflow-y-auto
      ...
      <ul>
        ...
        <li className="pl-8 py-2 font-semibold text-gray-700 dark:text-gray-400
          hover:bg-pink-200 dark-hover:bg-pink-500 mb-2 transition
          duration-500 ease-in-out">
          <a href="/BidProduct" className="focus:text-pink-500 dark-focus:text-pink-400
            focus:outline-none w-full transition duration-500 ease-in-out">
            <span className="flex items-center">
              <span className="ml-4 capitalize">BID</span>
            </span>
          </a>
        </li>
      ...
      <div className="pricing-plan-wrap lg:w-1/3 my-4 md:my-6">
        <div className="pricing-plan border-t-4 border-solid border-white bg-white text-center max-w-sm mx-auto hover:border-indigo-600 transition-colors duration-300">
          ...
          <div className="p-6">
            ...
            <div className="mt-6 py-4">
            <Link  onClick={ExpertPack}><button className="bg-indigo-600 text-xl text-white py-2 px-6 rounded hover:bg-indigo-700 transition-colors duration-300">Get PACK</button></Link>
            </div>
          </div>
        </div>
      </div>
